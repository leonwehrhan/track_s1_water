...
def assign_water_sites(trj_file, ref_file, w_loc_ids, w_names, ref_file_water=None, verbose=False):
    '''
    Assign one water oxygen atom to every specified water site for each frame.

    The assignment of waters to the sites is based on virtual particles created at
    the site positions. A neighbor list from the virtual sites is created to contain
    all neighboring water oxygens within a 2 Angstrom radius. The assignment covers
    the following cases for each frame:

    - case 1: The neighbor lists of all sites contains exactly one unique water molecule
        -> The water oxygen ids are saved to the output array.

    - case 2: One or more of the water molecules appear in two or more neighbor lists.
        -> Calculate distance of the water oxygen to all sites in which neighbor list it
        appears. Kick it out of all neigbor list but the one corresponding to the closest
        site. Exception: One of the sites' neighbor lists contains only this id, while all
        others have multiple. In this case the id is kept in that neighbor list and kicked out
        of the other.

    - case 3: After handling case 2, one or more neighbor lists contain more than one id.
        -> Calculate distance of all water oxygens of the neighbor list to the site. Keep
        the closest one.

    - case 4: One or more neighbor lists are empty (after handling case 2).
        -> Assign zero to the site.
    ...
    t.top.add_residue('VIR', t.top.chain(n_chains - 1))
    # add one virtual oxygen to last residue for each water site
    for i, name in enumerate(w_names):
        t.top.add_atom(f'VIR-{name}', md.element.oxygen, t.top.residue(t.n_residues - 1))
    ...
    for i_a, a in enumerate(t.top.residue(t.n_residues - 1).atoms):
        ...
        if not a.name == f'VIR-{w_names[i_a]}':
